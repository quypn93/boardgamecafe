...
namespace VRArcadeFinder.Services
{
    ...
    {
        Task<string?> DownloadAndSaveImageAsync(string imageUrl, string category = "arcades");
    ...
    public class ImageStorageService : IImageStorageService
    {
        ...
        private readonly string _arcadeImagesPath;
            ...
            IHttpClientFactory httpClientFactory)
        {
            ...
            // Get arcade images path from config, default to wwwroot/images if not configured
            _arcadeImagesPath = _configuration["ImageStorage:ArcadeImagesPath"] ?? "";
            if (string.IsNullOrEmpty(_arcadeImagesPath))
            {
                _arcadeImagesPath = Path.Combine(_environment.WebRootPath, "images");
            }
        }

        public async Task<string?> DownloadAndSaveImageAsync(string imageUrl, string category = "arcades")
        {
            try
            {
                ...
                if (!string.IsNullOrEmpty(_configuration["ImageStorage:ArcadeImagesPath"]))
                {
                    // Custom configured path
                    fullCategoryPath = Path.Combine(_arcadeImagesPath, category);
