...
namespace VRArcadeFinder.Models.Domain
{
    public class Arcade
    {
        ...
        // VR-specific fields
        [MaxLength(500)]
        public string? VRPlatforms { get; set; } // e.g., "Meta Quest, HTC Vive, PlayStation VR, Valve Index"

        public int? TotalVRStations { get; set; }

        public bool HasMultiplayerArea { get; set; } = false;

        public bool HasPartyRooms { get; set; } = false;
        ...
        [MaxLength(50)]
        public string? PremiumTier { get; set; } // "basic", "standard", "premium"
        ...
        public List<string> GetVRPlatformsList()
        {
            if (string.IsNullOrEmpty(VRPlatforms))
                return new List<string>();

            return VRPlatforms.Split(',', StringSplitOptions.RemoveEmptyEntries)
                .Select(p => p.Trim())
                .ToList();
        }
    }
