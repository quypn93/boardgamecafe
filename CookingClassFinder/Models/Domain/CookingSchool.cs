...
{
    /// <summary>
    /// Represents a cooking school or culinary studio that offers cooking classes
    /// </summary>
    public class CookingSchool
    {
        [Key]
        ...
        public int MaxStudentsPerClass { get; set; } = 12;
        public bool HasProfessionalKitchen { get; set; } = true;
        public bool OffersPrivateClasses { get; set; } = true;
        public bool OffersCorporateEvents { get; set; } = false;
        public bool OffersKidsClasses { get; set; } = false;
        public bool OffersCouplesClasses { get; set; } = false;
        public bool HasParking { get; set; } = false;
        public bool IsWheelchairAccessible { get; set; } = false;
        public bool ProvidesCertificates { get; set; } = false;
        public int? MinAge { get; set; } // Minimum age for classes

        // Cuisine specialties (JSON array)
        [Column(TypeName = "nvarchar(max)")]
        public string? CuisineSpecialties { get; set; } // JSON array: ["Italian", "French", "Asian"]
        ...
        public virtual ICollection<CookingClass> Classes { get; set; } = new List<CookingClass>();
        ...
        public List<string> GetCuisineSpecialtiesList()
        {
            if (string.IsNullOrEmpty(CuisineSpecialties))
                return new List<string>();

            try
            {
                return JsonSerializer.Deserialize<List<string>>(CuisineSpecialties) ?? new List<string>();
            }
            catch
            {
                return new List<string>();
        ...
        public void SetCuisineSpecialties(List<string> cuisines)
        {
            CuisineSpecialties = JsonSerializer.Serialize(cuisines);
        }

        public double GetAverageClassRating()
        {
            if (!Classes.Any()) return 0;
            var classesWithRating = Classes.Where(c => c.AverageRating.HasValue).ToList();
            if (!classesWithRating.Any()) return 0;
            return (double)Math.Round(classesWithRating.Average(c => c.AverageRating!.Value), 1);
        }
    }
